...
舒尔特方格是一种经典的注意力和眼球追踪训练工具，广泛用于脑力康复与认知提升领域。

**基本流程：预览 → 游戏 → 结果**

1. **选择难度**：初级（3×3）、中级（4×4）、高级（5×5）
2. **预览阶段**：方格中的数字全部可见，用户先记忆布局，倒计时结束后正式开始
3. **游戏阶段**：数字随机排布，按照从小到大的顺序依次点击（1, 2, 3 … N），计时开始
4. **完成阶段**：点完所有数字后自动进入结果页，展示得分、用时、准确率等指标
...
得分由以下几部分组成，整体设计对中老年更友好（不会因失误导致 0 分）：

- **完成奖励**：完成游戏即可获得基础完成奖励
- **时间惩罚**：用时越长扣分越多，但有平滑封顶
- **错误惩罚**：点错一次扣少量分，总惩罚上限为基础分的 40%
- **无失误奖励**：全程零错误，额外加分
- **连击奖励**：连续正确点击 3/5/8 个以上依次获得额外奖励

### 自适应推荐难度

> 仅对**已登录用户**启用，未登录时不请求后端、不显示推荐。

进入注意力训练页面时，系统会自动从后端拉取用户最近 10 条 attention 训练记录（接口：`GET /training/list?game_type=attention&page=1&page_size=10`），基于以下指标计算推荐难度：

- **avgSecPerCell**：历史平均每格耗时 = 总用时 ÷ 格子数
- **recent3Bad**：最近 3 次中表现较差（每格 ≥ 1.8 秒）的次数

**推荐规则（升级严格、降级敏感，保护中老年用户不被打击）：**

| 条件 | 操作 |
|------|------|
| 最近 3 次有 ≥2 次每格 ≥1.8 秒，或历史均值每格 ≥1.6 秒 | 推荐降低 1 个难度档位 |
| 最近 3 次有 ≥2 次每格 ≤0.9 秒，且历史均值 ≤1.0 秒 | 推荐提升 1 个难度档位 |
| 其他情况 | 维持当前平均难度 |

**重要说明：**
- 推荐难度仅作为**默认选中项**展示，用户可自由手动更改，不会强制切换
- 无训练历史时默认推荐初级（3×3），并自动延长预览时间

### 预览时间自适应

预览时间会根据历史表现**自动延长**，让反应稍慢的用户有更充裕的准备时间：
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/training/list?game_type=attention&page=1&page_size=10` | 拉取历史记录，用于自适应推荐 |
| POST | `/api/v1/training/submit` | 游戏完成后提交成绩（game_type=attention） |

后端 `TrainingRecord` 字段：`game_type`、`score`、`difficulty`（1/2/3）、`duration`（秒）、`created_at`。自适应逻辑基于 `score`、`duration`、`difficulty` 推导，**不依赖** errorCount 字段。

### 自测步骤（手动验证）

1. **未登录状态**：
   - 进入注意力训练页，不应发起 `/training/list` 请求
   - 不显示"推荐"标签和提示条
   - 游戏完成后不提交成绩，结果页正常展示

2. **已登录、无历史记录**：
   - 进入页面后推荐默认选中"初级（3×3）"
   - 点击"开始游戏"，预览倒计时约为 4.5 秒（基础 3.0 + 延长 1.5）
   - 游戏完成后成绩正常提交

3. **已登录、有历史记录（表现较好）**：
   - 多次完成高级（5×5）且每格用时 ≤0.9 秒后
   - 推荐档位应维持或升至更高难度
   - 预览倒计时较短（接近基础值）

4. **已登录、有历史记录（表现较差）**：
   - 多次完成中级（4×4）但每格用时 ≥1.8 秒后
   - 推荐档位应降为初级（3×3）
   - 预览倒计时自动延长

5. **成绩提交失败不影响结果页**：
   - 断网或后端异常时，游戏结果页仍正常展示得分、星级、统计信息
